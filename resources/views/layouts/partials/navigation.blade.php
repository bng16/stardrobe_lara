<nav class="bg-white border-b border-gray-100" x-data="{ mobileMenuOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @auth
                        @if(auth()->user()->hasRole('creator'))
                            <x-nav-link href="{{ route('creator.dashboard') ?? '#' }}" :active="request()->routeIs('creator.*')">
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                ...
                @else
                    <div class="space-x-4">
                        <a href="{{ route('login') }}" class="text-gray-500 hover:text-gray-700 transition duration-150 ease-in-out">{{ __('Login') }}</a>
                        <a href="{{ route('register') }}" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition duration-150 ease-in-out">{{ __('Register') }}</a>
                    </div>
                @endauth
            </div>
    ...
    <div :class="{'block': mobileMenuOpen, 'hidden': !mobileMenuOpen}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                @if(auth()->user()->hasRole('creator'))
                    <x-responsive-nav-link href="{{ route('creator.dashboard') ?? '#' }}" :active="request()->routeIs('creator.*')">
                        {{ __('Creator Dashboard') }}
                    </x-responsive-nav-link>
                @endif
